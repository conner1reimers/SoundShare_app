...
export const VALIDATOR_LOGIN = (val: any) => ({ type: VALIDATOR_TYPE_LOGIN, val: val });



export const validate = (value: any, validator: any) => {
  let isValid = true;
    ...
    if (validator.type === VALIDATOR_TYPE_LOGIN) {
      isValid = isValid && (/^\S+@\S+\.\S+$/.test(value) || value.trim().length <= validator.val);
    }
  return isValid;
};
