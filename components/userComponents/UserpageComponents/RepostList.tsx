...
const RepostList: React.FC = () => {
    const [refreshing, setRefreshing] = useState(false);
    
    const userInfo = useSelector((state: Root) => state.userPage.userReposts);
    const curOffset = useSelector((state: Root) => state.userPage.refreshOptions.offsetReposts);
    const refreshFinish = useSelector((state: Root) => state.userPage.refreshOptions.repostFinished);
        ...
        {userInfo &&
          ...
          {userInfo && userInfo.length === 0 && (
            <li className="user-no-sounds">
              <div>
                <p>
                  This user{` hasn't`} reposted any sounds yet.
                </p>
              </div>
            </li>
          )}
